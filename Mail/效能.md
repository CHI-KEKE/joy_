...
```sql
SELECT
    date_parse(date, '%Y/%m/%d') AS day_time,
    count(*) AS job_count
FROM "hk_prod_nmqv3"."view_worker_log"
WHERE date_parse(date, '%Y/%m/%d') >= DATE '2026-03-29'
    AND json_extract_scalar(_props, '$.jobname') = 'SendTemplateMailPriorityLow'
GROUP BY 1
ORDER BY 1
LIMIT 500;
```