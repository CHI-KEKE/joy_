...
```sql
select ShippingOrder_Code,ShippingOrderSlave_OuterCode,*
from ShippingOrder(nolock)
inner join ShippingOrderSlave(nolock)
on ShippingOrder_Id = ShippingOrderSlave_ShippingOrderId
where ShippingOrder_ValidFlag = 1
and ShippingOrder_SalesOrderId = 15425
```
...
EI 存在這 : `ShippingOrderSlave_OuterCode`
