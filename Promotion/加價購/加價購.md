...
1. [概念](#概念)
2. [加價購的資格要根據「折扣後金額」來判斷](#加價購的資格要根據折扣後金額來判斷)
3. [要在抵價券之後，點數折抵之前](#要在抵價券之後點數折抵之前)
4. [避免重複判斷與贈品干擾](#避免重複判斷與贈品干擾)
5. [促購前台金額 mapping](#促購前台金額-mapping)
6. [實際案例 - 貓腿](#實際案例---貓腿)
7. [排除加價購的方法](#排除加價購的方法)
...
**抵價券**會直接降低購物車金額。所以活動通常會定義為：「加價購門檻不含抵用券後金額」。
也就是說折價券折完後的小計還要滿 $1000，才能加購。如果你把加價購放在抵價券之前，就會錯誤地用「折前金額」判門檻。導致顧客折完後實際金額低於門檻但仍能加購，造成條件錯誤

**點數折抵**是顧客的支付行為，屬於「付款階段」的動作。但加價購是「購物階段」的活動（加入購物車時判定）。你必須在顧客抵點數前，先確定他是否有加購資格。否則點數折完後金額會下降，會出現「資格閃爍」的問題（前端 UI 顯示加購→再被移除）
...
**主商品：** 62227 有 SKU, 4.5 * 14 = 63，Flag：`"AddOnsSalepageMajor"`
**加購品：** 60393 貓腿, 6.66，Flag：`"AddOnsSalepageSub"`
  ...
  "SalepageSkuList": [
    {
      ...
      "CartExtendInfos": [
        {
          "RuleTypeDef": "AddOnsSalepageExtraPurchase", // 加價購
          "RuleId": 10000050,
