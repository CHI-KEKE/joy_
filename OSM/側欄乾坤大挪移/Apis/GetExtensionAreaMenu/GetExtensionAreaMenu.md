https://sms.qa1.hk.91dev.tw/Api/Home/GetExtensionAreaMenu?shopId=2&type=ExtensionArea



```csharp
var data = this._authService.GetCloudSidebarMenu(user.SupplierId, user.RolesId, user.UsersId, user.SystemName, cloudType, type, cleanCache, shopId)
                        .ExtensionArea;

```





```json
[
    {
        "CloudType": "Commerce",
        "Modules": [
            {
                "Id": 7,
                "Name": "優惠券",
                "Type": "ExtensionArea",
                "Functions": [
                    {
                        "Id": 728,
                        "Name": "優惠券設定",
                        "Url": "https://sms.qa1.hk.91dev.tw/ECoupon/List",
                        "IsShortcut": false,
                        "ShopIdList": [
                            2
                        ]
                    },
                    {
                        "Id": 424,
                        "Name": "優惠券報表",
                        "Url": "https://sms.qa1.hk.91dev.tw/Sidebar/ECouponReport",
                        "IsShortcut": false,
                        "ShopIdList": [
                            2
                        ]
                    }
                ]
            },
            {
                "Id": 20,
                "Name": "團購活動",
                "Type": "ExtensionArea",
                "Functions": [
                    {
                        "Id": 774,
                        "Name": "團購活動",
                        "Url": "https://sms.qa1.hk.91dev.tw/CommerceCloud/ReferrerPromotion",
                        "IsShortcut": false,
                        "ShopIdList": []
                    }
                ]
            },
            {
                "Id": 8,
                "Name": "會員積分",
                "Type": "ExtensionArea",
                "Functions": [
                    {
                        "Id": 409,
                        "Name": "會員積分設定",
                        "Url": "https://sms.qa1.hk.91dev.tw/LoyaltyPoint/ProfileSetting",
                        "IsShortcut": false,
                        "ShopIdList": []
                    },
                    {
                        "Id": 674,
                        "Name": "積分活動設定",
                        "Url": "https://sms.qa1.hk.91dev.tw/Promotion/RewardPointlist",
                        "IsShortcut": false,
                        "ShopIdList": []
                    },
                    {
                        "Id": 826,
                        "Name": "積分折抵排除商品設定",
                        "Url": "https://sms.qa1.hk.91dev.tw/CommerceCloud/RedeemPointExcludeCondition",
                        "IsShortcut": false,
                        "ShopIdList": []
                    },
                    {
                        "Id": 506,
                        "Name": "批次異動紅利點數",
                        "Url": "https://sms.qa1.hk.91dev.tw/BatchUpload/BatchModifyLoyaltyPoint",
                        "IsShortcut": false,
                        "ShopIdList": [
                            2
                        ]
                    },
                    {
                        "Id": 733,
                        "Name": "會員積分報表",
                        "Url": "https://sms.qa1.hk.91dev.tw/Sidebar/LoyaltyPointReport",
                        "IsShortcut": false,
                        "ShopIdList": []
                    }
                ]
            },
            {
                "Id": 9,
                "Name": "會員邀請活動",
                "Type": "ExtensionArea",
                "Functions": [
                    {
                        "Id": 695,
                        "Name": "會員邀請活動",
                        "Url": "https://sms.qa1.hk.91dev.tw/Promotion/PromoCodeMGOList",
                        "IsShortcut": false,
                        "ShopIdList": []
                    }
                ]
            },
            {
                "Id": 10,
                "Name": "優惠碼活動",
                "Type": "ExtensionArea",
                "Functions": [
                    {
                        "Id": 678,
                        "Name": "優惠碼活動",
                        "Url": "https://sms.qa1.hk.91dev.tw/Promotion/PromoCodeKOLList",
                        "IsShortcut": false,
                        "ShopIdList": []
                    },
                    {
                        "Id": 686,
                        "Name": "優惠碼付款報表設定",
                        "Url": "https://sms.qa1.hk.91dev.tw/ShopDefault/PromoCodePaymentExportSetting",
                        "IsShortcut": false,
                        "ShopIdList": []
                    }
                ]
            },
            {
                "Id": 23,
                "Name": "直播活動",
                "Type": "ExtensionArea",
                "Functions": [
                    {
                        "Id": 875,
                        "Name": "直播活動管理",
                        "Url": "https://91app-live.qa1.hk.91dev.tw/live-events",
                        "IsShortcut": false,
                        "ShopIdList": []
                    },
                    {
                        "Id": 876,
                        "Name": "直播平台設定",
                        "Url": "https://91app-live.qa1.hk.91dev.tw/stream-settings",
                        "IsShortcut": false,
                        "ShopIdList": []
                    }
                ]
            },
            {
                "Id": 14,
                "Name": "商品評價管理",
                "Type": "ExtensionArea",
                "Functions": [
                    {
                        "Id": 706,
                        "Name": "商品評價管理",
                        "Url": "https://sms.qa1.hk.91dev.tw/SalePageComment/Index",
                        "IsShortcut": false,
                        "ShopIdList": []
                    },
                    {
                        "Id": 709,
                        "Name": "商品評價設定",
                        "Url": "https://sms.qa1.hk.91dev.tw/ShopDefault/ToggleCommentSetting",
                        "IsShortcut": false,
                        "ShopIdList": []
                    }
                ]
            },
            {
                "Id": 22,
                "Name": "定期購",
                "Type": "ExtensionArea",
                "Functions": [
                    {
                        "Id": 731,
                        "Name": "定期購管理",
                        "Url": "https://regularpurchase-manager.qa1.hk.91dev.tw/admin/controllers/regular/product.php",
                        "IsShortcut": false,
                        "ShopIdList": []
                    }
                ]
            }
        ]
    },
    {
        "CloudType": "Marketing",
        "Modules": [
            {
                "Id": 21,
                "Name": "夥伴市集",
                "Type": "ExtensionArea",
                "Functions": [
                    {
                        "Id": 847,
                        "Name": "夥伴市集",
                        "Url": "https://analytics.qa1.hk.91dev.tw/#/growth/partnerMarket",
                        "IsShortcut": false,
                        "ShopIdList": []
                    }
                ]
            },
            {
                "Id": 1,
                "Name": "客戶關係管理(CRM)",
                "Type": "ExtensionArea",
                "Functions": [
                    {
                        "Id": 398,
                        "Name": "會員機制",
                        "Url": "https://sms.qa1.hk.91dev.tw/CrmMemberTier/Settings",
                        "IsShortcut": false,
                        "ShopIdList": [
                            2
                        ]
                    },
                    {
                        "Id": 396,
                        "Name": "會員禮",
                        "Url": "https://sms.qa1.hk.91dev.tw/VipMemberPresent/List",
                        "IsShortcut": false,
                        "ShopIdList": [
                            2
                        ]
                    },
                    {
                        "Id": 397,
                        "Name": "會員首下載禮設定",
                        "Url": "https://sms.qa1.hk.91dev.tw/AppFirstDownload/List",
                        "IsShortcut": false,
                        "ShopIdList": [
                            2
                        ]
                    },
                    {
                        "Id": 738,
                        "Name": "會員禮管理",
                        "Url": "https://sms.qa1.hk.91dev.tw/Marketing/VipMemberPresent",
                        "IsShortcut": false,
                        "ShopIdList": [
                            2
                        ]
                    }
                ]
            }
        ]
    },
    {
        "CloudType": "OMO",
        "Modules": [
            {
                "Id": 6,
                "Name": "店員幫手",
                "Type": "ExtensionArea",
                "Functions": [
                    {
                        "Id": 413,
                        "Name": "店員幫手設定",
                        "Url": "https://sms.qa1.hk.91dev.tw/Sidebar/LocationWizard",
                        "IsShortcut": false,
                        "ShopIdList": [
                            2
                        ]
                    },
                    {
                        "Id": 624,
                        "Name": "店員幫手報表",
                        "Url": "https://sms.qa1.hk.91dev.tw/Sidebar/LocationWizardReport",
                        "IsShortcut": false,
                        "ShopIdList": [
                            2
                        ]
                    }
                ]
            }
        ]
    }
]
```